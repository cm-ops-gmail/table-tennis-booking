...
| Booking cancelled | Owner cancels | Updates that Calendar event's description to matching "cancelled" wording, then deletes it — Calendar's own cancellation notice to the guests carries that text. Every player also gets an explicit cancellation email (Calendar's own notice alone isn't reliable enough — the organizer in particular never gets a self-notification), and line managers and HR get a "cancelled" email too, same grouping as above. |
...
Those emails (manager/HR always, plus every player on a cancellation) are
queued by the app and just need sending. The feedback reminder and the
calendar sync are different: both are driven by
